defmodule EcommerceApi.AccountsTest do
  ...
  describe "users" do
    ...
    test "authenticate_user/2 authenticates the user" do
      user = user_fixture()
      assert {:error, "Wrong username or password"} = Accounts.authenticate_user("wrong email", "")
      assert {:ok, authenticated_user, token} = Accounts.authenticate_user(user.email, @valid_attrs.password)
